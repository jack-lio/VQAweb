...
class Server(object):
    ...
    def inference(self, dataloader):
        ...
        print("[DEBUG] result keys =", [list(data.keys()) for data in result_list])
    ...
    def log_classification_result(
        ...
        print("[DEBUG] result_dict:", self.result_dict[self.epoch][data_split])
        if data_split == 'train':
            result_list = self.result_dict[self.epoch][data_split]
            loss = np.mean([data['loss'] for data in result_list])

            # 根據結果 key 判斷是分類還是回歸
            if 'acc' in result_list[0]:
                acc = np.mean([data['acc'] for data in result_list])
                uar = np.mean([data['uar'] for data in result_list])
                top5_acc = np.mean([data['top5_acc'] for data in result_list])
                f1 = np.mean([data['f1'] for data in result_list])
                logging.info(f'{data_split} set, Loss: {loss:.3f}, Macro-F1: {f1:.2f}%, Top-1 Acc: {acc:.2f}%')
            else:
                rmse = np.mean([data['rmse'] for data in result_list])
                mae = np.mean([data['mae'] for data in result_list])
                logging.info(f'{data_split} set, Loss: {loss:.3f}, RMSE: {rmse:.3f}, MAE: {mae:.3f}')

        else:
            result_dict = self.result_dict[self.epoch][data_split]
        ...
        if 'acc' in result_list[0]:
            ...
            self.log_writer.add_scalar(f'RMSE/{data_split}', rmse, self.epoch)
            self.log_writer.add_scalar(f'MAE/{data_split}', mae, self.epoch)
